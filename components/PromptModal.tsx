...
export const PromptModal: React.FC<PromptModalProps> = ({ isOpen, language, mode, onClose, onSubmit }) => {
  ...
  const handleSubmit = async (e?: React.FormEvent) => {
    ...
    try {
      ...
      console.error(err);
      setError(isZh ? '魔法失败了！请重试。' : 'Magic failed! Please try again.');
    } finally {
      setIsLoading(false);
    }
  };
      ...
      <div className={`bg-white rounded-3xl shadow-2xl w-full max-w-lg flex flex-col border-4 ${isCreate ? 'border-sky-100' : 'border-amber-100'} animate-in fade-in zoom-in duration-200 scale-95 sm:scale-100 overflow-hidden`}>
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
                <h2 className="text-xl font-extrabold text-slate-800">
                    {isCreate ? (isZh ? 'AI 新建作品' : 'AI Create') : (isZh ? 'AI 重塑作品' : 'AI Morph')}
                </h2>
        ...
        <div className="p-6 bg-white">
          <p className="text-slate-600 font-semibold mb-4">
            {isCreate 
                ? (isZh ? "你想创造什么样的 3D 作品？" : "What kind of 3D work do you want to create?") 
                : (isZh ? "你想如何重塑当前的方块？" : "How do you want to reshape the blocks?")}
          </p>
          ...
          <form onSubmit={handleSubmit}>
            <textarea 
              ...
              placeholder={isCreate 
                ? (isZh ? "例如：一个中世纪城堡，一个巨大的机器人，或者一篮子水果..." : "e.g., A medieval castle, a giant robot, or a basket of fruit...") 
                : (isZh ? "例如：把它变成一辆车，做一个金字塔，或者建一个笑脸..." : "e.g., Turn it into a car, make a pyramid, or build a smiley face...")}
              disabled={isLoading}
