...
export const JsonModal: React.FC<JsonModalProps> = ({ isOpen, language, onClose, data = '', isImport = false, onImport }) => {
  ...
  const handleImportClick = () => {
      if (!importText.trim()) {
          setError(isZh ? '请先粘贴 JSON 数据。' : 'Please paste JSON data first.');
          return;
      }
      try {
          ...
          setError(isZh ? '无效的 JSON 格式，请检查输入内容。' : 'Invalid JSON format, please check your input.');
      }
  };
      ...
      <div className="bg-white rounded-3xl shadow-2xl w-full max-w-2xl flex flex-col h-[70vh] border-4 border-slate-100 animate-in fade-in zoom-in duration-200 scale-95 sm:scale-100">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
                <h2 className="text-xl font-extrabold text-slate-800">
                    {isImport ? (isZh ? '导入蓝图' : 'Import Blueprint') : (isZh ? '导出并分享你的作品' : 'Export & Share Your Work')}
                </h2>
        ...
        <div className="flex-1 p-6 overflow-hidden bg-slate-50/50 flex flex-col relative">
          <textarea 
            ...
            placeholder={isImport ? (isZh ? "在此粘贴你的体素 JSON 数据..." : "Paste your voxel JSON data here...") : ""}
          ...
          {isImport ? (
              ...
              <>
                ...
                >
                    ...
                    {isCopied ? (isZh ? '已复制!' : 'Copied!') : (isZh ? '全部复制' : 'Copy All')}
                </button>
                <button 
                    onClick={onClose}
